...
function ArchiveEmailsandFolders{
    param(
            [string]$userfullname
        )
}
...
if (Test-Path $UserListPath) {
    ...
    foreach ($user in $UserList) {
        ...
        if ($userObject.Enabled -eq $false) {
            ...
            if($Archive -eq "Yes" ){
                    try{
                        #copy and paste the user folder to the archive location 
                        ArchiveEmailsandFolders -userfullname $user
                        DeleteUserFolders -userfullname $user 
                    }catch{
                        #
                        DeleteUserFolders -userfullname $user 
                    }
